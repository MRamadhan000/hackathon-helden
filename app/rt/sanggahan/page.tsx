  ...
  "2026": [
    {
      ...
      alasanSanggahan:
        "Pak RT, mohon bantuan perbaikan ejaan nama saya di data desa menjadi 'Siti Aminah, S.Pd' agar sesuai dengan ijazah dan KTP elektronik.",
  ...
  "2025": [
    {
      ...
      alasanSanggahan:
        "Pak RT, perbaikan alamat rumah saya di data desa menjadi RT 03 / RW 01.",
  ...
  "2026": [
    {
      ...
      skorSistem: 75,
      alasanWarga:
        "Pak RT, atap dapur kami bocor parah dan dinding kayu sudah lapuk dimakan rayap, serta belum punya jamban pribadi. Mohon dibantu tinjau ulang untuk pengusulan BLT.",
...
export default function HalamanSanggahan() {
  ...
  const [listPenduduk, setListPenduduk] = useState<SanggahanDataPenduduk[]>(
    mockSanggahanPendudukPerTahun[tahunPeriode] || [],
  );
  const [listRumah, setListRumah] = useState<SanggahanKondisiRumah[]>(
    mockSanggahanRumahPerTahun[tahunPeriode] || [],
  );

  const [notif, setNotif] = useState("");

  const handleAjukanPenduduk = (id: string) => {
    ...
    setNotif(
      "Sukses: Permohonan perbaikan data warga berhasil diteruskan ke Sekdes!",
    );
    setTimeout(() => setNotif(""), 4000);
  };

  const handleAjukanRumah = (id: string) => {
    setListRumah((prev) =>
      prev.map((item) =>
        item.id === id ? { ...item, status: "Diajukan ke Sekdes" } : item,
      ),
    ...
    setNotif(
      "Sukses: Laporan sanggahan kondisi rumah warga berhasil diteruskan ke Sekdes!",
    );
    setTimeout(() => setNotif(""), 4000);
  };

  const pendingRumahCount = listRumah.filter(
    (r) => r.status === "Pending",
  ).length;
  const pendingPendudukCount = listPenduduk.filter(
    (p) => p.status === "Pending",
  ).length;
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-10 space-y-6 font-sans antialiased">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Tinjau dan tindak lanjuti laporan mandiri dari warga sebelum
              diteruskan ke Sekretaris Desa.
            </p>
          </div>
        ...
        {/* TAB SWITCHER DENGAN BADGE NOMOR SANGAT TEGAS & KONTRAS */}
        <div className="flex border-b border-slate-200/80 gap-2 bg-white p-1.5 rounded-2xl border shadow-2xs">
          ...
          >
            {/* BADGE NOMOR 1 WARNA KONTRAS */}
            <span
              className={`w-5 h-5 rounded-lg flex items-center justify-center text-[11px] font-black ${
                activeTab === "rumah" ? "" : ""
              }`}
            ></span>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-5 animate-in fade-in duration-150">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <span className="text-xs text-slate-400 font-medium">
                Total: {listRumah.length} Berkas
              </span>
            </div>

            <div className="space-y-4">
              {listRumah.length > 0 ? (
                listRumah.map((item, index) => (
                  <div
                    key={item.id}
        ...
        {activeTab === "penduduk" && (
          <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-5 animate-in fade-in duration-150">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <span className="text-xs text-slate-400 font-medium">
                Total: {listPenduduk.length} Berkas
              </span>
            </div>

            <div className="space-y-4">
              {listPenduduk.length > 0 ? (
                listPenduduk.map((item, index) => (
                  <div
                    key={item.id}
                    className="p-5 bg-slate-50 border border-slate-200/80 rounded-2xl space-y-4"
                  >
                      ...
                      <div className="flex items-start gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 font-mono mt-0.5">
                            Kategori:{" "}
                            <strong>{item.jenisKetidakcocokan}</strong> • Tgl:{" "}
                            {item.tanggalMasuk}
                          </p>
                        </div>
                      </div>
                ...
                <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-slate-400 text-xs">
                  Tidak ada permohonan perbaikan data diri pada tahun{" "}
                  {tahunPeriode}.
                </div>
              )}
            </div>
        ...
        <div className="pt-4">
          <CardSanggahan
            ...
            sanggahanPendudukList={listPenduduk}
            sanggahanRumahList={listRumah}
